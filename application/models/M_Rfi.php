...
class M_Rfi extends CI_Model {
	...
	{ 
		if($a==""){$tm_project=$this->db->get('tabrfi')->result();}else
...
UNION SELECT DISTINCT (F1.itemid)a, (F1.itemsize)b FROM (SELECT tabitemcode.itemid,tabschedule.itemsize FROM tabschedule LEFT JOIN tabitemcode ON tabschedule.itemsize=tabitemcode.name WHERE tabschedule.itemcode LIKE '".$a."%') F1";
	...
	{	$a =$this->session->userdata('roadid');
		...
		$object = [];
			...
			if (count($object) > 0) {
				return $this->db->insert_batch('tabrfi',$object);}else{ return false;}
	...
	public function save_Rfi_action()	{
		...
		if ($b && $apprv=='Approved'){
			...
			if ($tinfo['a']=='0'){// no rfi sub item exist
				...
				if ($tinfo['unit']=='Km'){// linear item							
					if ($phycode['s']==0) {$b=$this->insert_linear_progress($this->input->post('chd'),$this->input->post('rfiid2'));}
				}
				if ($tinfo['unit']=='No'){// Point item
					if ($phycode['s']==0) {$b=$this->insert_point_progress($this->input->post('chd'),$this->input->post('rfiid2'));}
				}
				if ($tinfo['unit']=='Stage'){// Stage item
					if ($phycode['s']==0) {$b=$this->insert_Stage_progress($this->input->post('chd'),$this->input->post('rfiid2'));}
				}
			}
		}
		return $b;
	...
	$object2 = [];			
		for ($j = 0; $j <$i; $j++) {
			if($j<=$sp){$nid=$a.'-'.$st[0];}else{$nid=$a.'-'.$st[$j-$sp];}
			$object2[]=array(
					'rid'=>$rid,
					'stageid'=>$nid,
					'status'=>'COMPLETED',
			...
			if($j>0){$fstatus=$fstatus.','.'COMPLETED';	}	
			}		
	...
	public function verify_drawing($ch,$id)	{	
		...
		$b = explode('-', $ch);
		$ch1=$b[0];
		if(count($b)>1){$ch2=$b[1];}else{$ch2=$b[0];}	
