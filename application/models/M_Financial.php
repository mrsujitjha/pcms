...
class M_Financial extends CI_Model {
	...
	{ $b=$this->session->userdata('userid');
		if($a==""){
			$querystring="SELECT n.*,tabfinprocess.flow FROM (SELECT fincode, max(sn)as sn FROM tabfinance group by fincode) as m JOIN (SELECT fincode, sn,pkgid,mdate,amount,descrip,fromid,toid,status FROM tabfinance) as n ON m.fincode=n.fincode  and m.sn=n.sn JOIN tabfinprocess ON tabfinprocess.pkgid=n.pkgid WHERE (n.fromid=". $b ." OR n.toid=".$b.")";
		}
		if($a=="All"){$querystring="SELECT n.*,tabfinprocess.flow FROM (SELECT fincode, max(sn)as sn FROM tabfinance group by fincode) as m JOIN (SELECT fincode, sn,pkgid,mdate,amount,descrip,fromid,toid,status FROM tabfinance) as n ON m.fincode=n.fincode  and m.sn=n.sn JOIN tabfinprocess ON tabfinprocess.pkgid=n.pkgid";
		}
		else{
			$querystring="SELECT n.*,tabfinprocess.flow FROM (SELECT fincode, max(sn)as sn FROM tabfinance group by fincode) as m JOIN (SELECT fincode, sn,pkgid,mdate,amount,descrip,fromid,toid,status FROM tabfinance) as n ON m.fincode=n.fincode  and m.sn=n.sn JOIN tabfinprocess ON tabfinprocess.pkgid=n.pkgid WHERE (n.fromid=". $b ." OR n.toid=".$b." OR ".$b."=1) AND n.pkgid='".$a."'";
		}
	...
	{ $mresult=1;
		if  (strpos($a, '%') !== false) {$f=str_replace(['min', '%'], '', $a);$mresult =$f*$mv /100; }else
			{if (strpos($a, 'min') !== false) {$f=str_replace(['min'], '', $a);$mresult =$f;}}	
		return $mresult;
	}


	public function get_sps_activity($a)
	{ 	if($a==""){$myquery="SELECT *,(SELECT CONCAT(fullname,'(',level,')') from user where user_code=tabfinance.fromid) as fuser,(SELECT CONCAT(fullname,'(',level,')') from user where user_code=tabfinance.toid) as tuser FROM `tabfinance`";
	}else{$myquery="SELECT *,(SELECT CONCAT(fullname,'(',level,')') from user where user_code=tabfinance.fromid) as fuser,(SELECT CONCAT(fullname,'(',level,')') from user where user_code=tabfinance.toid) as tuser FROM `tabfinance`WHERE fincode='".$a."'";
		}
	...
	{if($auto==0){$amt=$this->input->post('amt');}else{$amt=$auto;}
